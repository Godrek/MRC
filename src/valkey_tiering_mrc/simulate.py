...
TRUE_LFU_POLICY_LABEL = "true_lfu_global_frequency_lru_tiebreak"


@njit(cache=True)
def _simulate_true_lfu_one_capacity_exclude_first_touch(
    ...
    seen = np.zeros(n_unique, dtype=np.uint8)

    is_resident = np.zeros(n_unique, dtype=np.uint8)
    position = np.full(n_unique, -1, dtype=np.int64)
    ...
    for t in range(n):
        ...
        hit = is_resident[k] == 1
        if not hit and capacity_bytes > 0 and sz <= capacity_bytes:
            is_resident[k] = 1
    ...
    return int(obj_miss), int(byte_miss), int(measured_accesses), int(measured_requested_bytes)


@njit(cache=True)
def _simulate_true_lfu_one_capacity_cyclic(
    dense_keys: np.ndarray,
    key_size: np.ndarray,
    capacity_bytes: int,
    n_unique: int,
...
def compute_true_lfu_mrc(
    ...
    measured_accesses = None
    measured_requested_bytes = None
    ...
    for idx, cap in enumerate(bytes_grid):
        if progress and (idx == 0 or (idx + 1) % progress_step == 0 or idx == fractions.size - 1):
            print(f"    [{workload}] LFU capacity {idx + 1}/{fractions.size} (cap_bytes={int(cap)})")
        if measurement_mode == "exclude_first_touch":
            om, bm, ma, mbytes = _simulate_true_lfu_one_capacity_exclude_first_touch(
                dense_keys, key_size, int(cap), n_unique, int(keys.size)
            )
        elif measurement_mode == "cyclic":
            om, bm, ma, mbytes = _simulate_true_lfu_one_capacity_cyclic(
                dense_keys, key_size, int(cap), n_unique, int(keys.size)
            )
        else:
            raise ValueError(f"unknown measurement_mode: {measurement_mode}")

        if measured_accesses is None:
            measured_accesses = ma
            measured_requested_bytes = mbytes

        if ma == 0:
            obj_miss_arr[idx] = np.nan
            byte_miss_arr[idx] = np.nan
        else:
            obj_miss_arr[idx] = om / ma
            byte_miss_arr[idx] = bm / max(mbytes, 1)

    measured_accesses = int(measured_accesses or 0)
    measured_requested_bytes = int(measured_requested_bytes or 0)
    ...
    return lru.TraceMRC(
        ...
        first_touch_accesses=int(keys.size) - measured_accesses,
...
def simulate_policy_mrc_for_trace(
    ...
    policy_label = f"{TRUE_LFU_POLICY_LABEL}_{measurement_mode}"
    forward = pd.DataFrame(_mrc_to_forward_rows(mrc, policy_label))
    inverse = pd.DataFrame(_mrc_to_inverse_rows(mrc, policy_label))
...
def simulate_true_lfu_for_traces(
    ...
    fwd_csv = out_dir / f"true_lfu_{measurement_mode}_object_and_byte_mrc_curves.csv"
    inv_csv = out_dir / f"true_lfu_{measurement_mode}_inverse_mrc_curves.csv"
